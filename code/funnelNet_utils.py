    ...
    def __init__(self, settings_dict):
        ...
        for idx in range(self.set['total_instances']):
            ...
            # ALEX NET
                ...
                if not self.set['CCA']:
                    self.maps.append( get_AlexNet_maps(self.set['bias'], self.set['all_to_all'], self.set['total_instances']) )
...
def epoch_train_funnelNet(model, map_weight, train_loader, optimizer, mixup_alpha, device):
    ...
    Outputs: - train_loss_full (final loss term per batch)               [batches x 1]
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for idx in range(N):
            if mixup_alpha > 0:
                loss_entropy[idx] = lam * F.nll_loss(probabilities[idx], target) + (1-lam) * F.nll_loss(probabilities[idx], shuffled_target)
            else:
            ...
            if not model.set['CCA']:
                if not model.set['all_to_all']:
                    ...
                    for idx2 in range(N-1):
                        ...
                        loss_mapping[idx] += ((mapped_acts[idx_map] - activations[idx_act[idx_map]])**2).mean() / (N-1)
            else:
                if not model.set['all_to_all']:
                    map_w_idx = 2*idx
                    idx2 = (idx+1) % N
...
def test_funnelNet(model, map_weight, test_loader, device):
    ...
    test_loss_full = sum(test_loss_entropy)/len(test_loss_entropy) + map_weight * sum(test_loss_mapping)/len(test_loss_mapping)
...
def get_settings_dict(opt):
    ...
    :param opt: parameters object returned by ArgumentParser.parse_args(). 
    :return: dictionary containing as keys and values the parameter names and values.
    """
...
def get_model_name(settings_dict):
    ...
    :param settings_dict: parameter dictionary returned by get_settings_dict. 
    :return: string used for saving models.
    """
    model_name = '%s_%d_%s_%d_drop_%.2f' %(settings_dict['type_of_model'], 
                                           settings_dict['total_instances'], 
                                           settings_dict['dataset_name'], 
                                           settings_dict['percent_corrupt'], 
...
def get_map_weights(type_of_model, epochs, mapweight_params):
    ...
    Inputs:  - type_of_model ('funnelNet', 'baseline', or 'funnelNetMesh')
             - epochs (the number of training epochs)
             - mapweight_params (contains the 4 parameters {start value}{threshold value}{stepfactor 1}{stepfactor 2})
    Outputs: - map_weights (list of map_weight values, one for each epoch) 
    '''
...
def test_funnelNet_ablations(model, percent_ablate, test_loader, device):
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            ...
            pred_all = torch.zeros([target.shape[0],N],dtype=target.dtype).to(device)
            for idx in range(N):
                chosen_indices = np.random.choice([0, 1], size=(activations[idx].shape), p=[percent_ablate/100, 1-percent_ablate/100])
                activations[idx] = activations[idx] * torch.tensor(chosen_indices, dtype=activations[idx].dtype).to(device)
                ###############################################################################################
