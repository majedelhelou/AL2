...
parser.add_argument("--permute_heads",
                    type=bool,
                    default=False,
                    help="if True, at inference heads are permuted randomly")

# Data settings:
parser.add_argument("--dataset_name",
                    type=str,
                    default='MNIST',
                    help="supported: MNIST, FashionMNIST, CIFAR10, CIFAR100")
...
def main():
    ...
    ####################################################################################################
    classes = dict(MNIST=my_MNIST, FashionMNIST=my_FashionMNIST, CIFAR10=my_CIFAR10, CIFAR100=my_CIFAR100)
    normalizations = dict(MNIST=[(0.1307, ), (0.3081, )],
    ...
    if opt.arch_type == 'smallNet':
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(*normalizations[opt.dataset_name])])
    ...
    train_loader = torch.utils.data.DataLoader(classes[opt.dataset_name]('../data_{}'.format(opt.dataset_name),
                                                                         train=True,
                                                                         download=True,
                                                                         transform=transform,
    ...
    test_loader = torch.utils.data.DataLoader(classes[opt.dataset_name]('../data_{}'.format(opt.dataset_name),
                                                                        train=False,
                                                                        transform=transform),
    ...
    print('Initializing ~%s~ with: %d instances, %s architecture' %
          (opt.type_of_model, opt.total_instances, opt.arch_type))
    ...
    for epoch in range(opt.epochs):
        ...
        ##~~ TRAIN ~~##
        start = timer()
        [train_loss_full, train_loss_entropy,
         train_loss_mapping] = epoch_train_funnelNet(model_funnel, map_weights[epoch], train_loader, optimizer_funnel,
                                                     opt.mixup_alpha, device)
        ...
        ##~~ TEST ~~##
        [test_loss_full, test_loss_entropy, test_loss_mapping, test_accuracy,
         test_accuracy_ensemble] = test_funnelNet(model_funnel, map_weights[epoch], test_loader, device)
        ...
        print('    Epoch #%3d/%d: %.4f%%  ML: %.5f  (%.1f sec)' %
              (epoch, opt.epochs - 1, sum(test_accuracy) / len(test_accuracy), np.asarray(train_loss_mapping).mean(),
               end - start))
        ...
        if (opt.save_model) and (epoch % 50 == 0):
            results_dict = {
                'all_train_loss_full': np.asarray(all_train_loss_full),
