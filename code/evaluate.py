...
representations. This script reads the results of type epoch_{number}.pt. 
...
try:
    ...
    print('Need to clone svcca directory to ../../')
    print(e)
    sys.exit() 
...
def get_svcca_similarity(acts1, acts2, K=20, verbose=False, epsilon=None): 
    """ Compute svcca similarity, copied from tutorial. """
...
def get_activations(type_of_models, opt_dict, test_batch_size, epochs, results_dir=RESULTS_DIR):
    ...
    if 'MNIST' in opt_dict['dataset_name']:
        test_loader = torch.utils.data.DataLoader(
            my_MNIST('../data_MNIST', train=False, download=True, transform=transforms.Compose([
                    transforms.ToTensor(),
        ...
        test_loader = torch.utils.data.DataLoader(
            my_CIFAR10('../data_CIFAR10', train=False, download=True, transform=transforms.Compose([
                    transforms.ToTensor(),
                    transforms.Normalize((0.1307,), (0.3081,))])),
                    batch_size=test_batch_size, shuffle=False)
        size = [50, 5, 5]
    ...
    print('WARNING: We are only reading the first batch! We could potentially read all and do an average?')
    ...
    for this_model_name in type_of_models:
        ...
        for epoch in epochs:
            ...
            #map_weight = map_weights[epoch]
            with torch.no_grad():
                for data, target in test_loader:
                    data, target = data.to(device), target.to(device)
                    __, this_acts, mapped_acts = this_model(data)
                    # for now only look at the first activation. 
                    break
            print('read only 1 of {} for {} epoch {}'.format(
                len(test_loader), this_model_name, epoch))
...
def compute_similarity_measures(models_to_compare, activations, epochs):
    ...
    :param models_to_compare: list of model names to compare. 
    :param activations: dictionary of activations.
    :param epochs: epochs to use

    :return: dataframe with all similarity measures computed.
    '''
    plot_intermediate = False #True
    results = pd.DataFrame(columns=['epoch', 
                                    'this_model', 'other_model',
                                    'this_idx', 'other_idx', 
                                    'mean', 'svcca', 'pwcca', 'all_coeffs'])

    try:
        shape = activations['baseline'][epochs[0]][0].shape
    except KeyError:
        print(epochs)
        print(activations['baseline'].keys())
    ...
    for epoch in epochs:
        for i, this_model in enumerate(models_to_compare):
            for other_model in models_to_compare[i:]:
                ...
                # Can compare the mapped activations instead of normal activations for funnelNet. 
                #this_acts = mapped_activations if this_name=='funnelNet' else activations
                #other_acts = mapped_activations if other_name=='funnelNet' else activations
                ...
                # similarities
                #print('input shape:', this_act.T.shape, 'should be 50 x datapoints')
                ...
                print('epoch {}, {} to {}: {:.4f}'.format(epoch, this_model, other_model, mean_sim))
...
if __name__ == "__main__":
    ...
    #epochs = range(0, 501, 10) # all epochs
    epochs = range(0, 501, 100) # some epochs
    ...
    fname = f'../results/activations_{test_batch_size}.pkl'
    ...
    fname = f'../results/results_{test_batch_size}.pkl'
